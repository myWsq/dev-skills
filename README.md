...
The verification work is split deliberately: the delegated executor runs only fast, in-process checks (unit tests, typecheck, lint) as its development feedback loop; everything needing a runtime environment is acceptance-tier — e2e/UI test suites, anything requiring a running app, browser, or external service, a verify skill, verify-fix loops — and runs once, by the orchestrator, after the code review passes. The dividing line is the runtime environment, not the command's name. The executor's self-verification is never accepted as evidence, so heavyweight verification inside it would be pure duplicated cost.
