...
1. Executor preface:
   ...
   > You are executing the plan below. It is an outcome contract, not a step-by-step script: understand the Requirement and the Decisions & tradeoffs, then design the implementation yourself against the live code. Follow every recorded decision; if you must deviate, say so and justify it in your report. Work milestone by milestone and run each milestone's validation before continuing — mechanical checks only (tests, typecheck, lint). Do not run project-level verification or acceptance flows: no verify skill, no verify-fix loops, no launching and driving the app; final acceptance happens outside this session after code review. Change only in-scope files. Do not edit `plans/README.md`. Stop on any STOP condition. Commit work on the current branch as you go. When finished, report what changed, validation results, commits, and any deviations from the recorded decisions. Only claim results backed by commands you actually ran.
