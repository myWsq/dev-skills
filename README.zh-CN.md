...
角色是刻意分离的:受委派的执行方**只负责实现**——写代码和计划要求的测试,不运行任何验证命令。所有检查都在主 agent 一侧执行,从便宜到昂贵排序:机械检查(单元测试、类型检查、lint)、代码评审、验收层验证——e2e/UI 套件、任何需要运行中的应用/浏览器/外部服务的命令、verify 流程。检查失败以携带错误输出的具体修订意见发回执行方。执行方的自我验证本来就不会被采信为证据,而且自我验证会诱发 fix-loop,把精力从实现本身抽走。
