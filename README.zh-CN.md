...
核验工作是刻意分层的:受委派的执行方只跑计划里的机械性里程碑检查(测试、类型检查、lint),作为它的开发反馈回路;项目级验证与验收流程——verify 流程、verify-fix 循环、启动并驱动应用——由主 agent 在代码评审通过后统一执行一次。执行方的自我验证从不被采信为证据,在它内部跑重型验证只是纯粹的重复成本。
